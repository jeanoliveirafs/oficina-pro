...
export const ReceitaForm = ({ receitaInicial, onFinished }: ReceitaFormProps) => {
  const queryClient = useQueryClient();
  const isEditMode = !!receitaInicial;
  ...
  const form = useForm<ReceitaFormValues>({
    ...
    defaultValues: {
      ...
      valor: receitaInicial?.valor ?? 0,
      data: receitaInicial?.data ? new Date(receitaInicial.data) : new Date(),
      categoria_id: receitaInicial?.categoria_id ?? null,
      status: receitaInicial?.status ?? "recebido",
    },
  });
  ...
  const onSubmit = async (values: ReceitaFormValues) => {
    if (!oficinaId) {
